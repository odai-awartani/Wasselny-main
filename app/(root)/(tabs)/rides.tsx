...
export default function Rides() {
  ...
  const fetchRides = useCallback(async () => {
    ...
    // Check cache first
    const cachedData = await getCachedRidesData();
    if (cachedData) {
      setUpcomingRides(cachedData.upcomingRides);
      setPastDriverRides(cachedData.pastDriverRides);
      setPastPassengerRides(cachedData.pastPassengerRides);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Fetch passenger rides (where user has an accepted ride request)
      ...
      const sortedRides = allRides.sort((a, b) => {
        const dateA = new Date(a.ride_datetime.split(' ')[0].split('/').reverse().join('-') + ' ' + a.ride_datetime.split(' ')[1]);
        const dateB = new Date(b.ride_datetime.split(' ')[0].split('/').reverse().join('-') + ' ' + b.ride_datetime.split(' ')[1]);
        return dateB.getTime() - dateA.getTime();
      });

      const upcoming = sortedRides.filter((ride) => {
        const rideDate = new Date(ride.ride_datetime.split(' ')[0].split('/').reverse().join('-') + ' ' + ride.ride_datetime.split(' ')[1]);
        return rideDate > now;
      });

      const past = sortedRides.filter((ride) => {
        const rideDate = new Date(ride.ride_datetime.split(' ')[0].split('/').reverse().join('-') + ' ' + ride.ride_datetime.split(' ')[1]);
        return rideDate <= now;
      });
